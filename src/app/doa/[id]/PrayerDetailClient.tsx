    ...
    <div className="min-h-screen bg-[#faf7f2]">
      ...
      <header className="glass sticky top-0 z-50 border-b border-stone-200/60">
        <div className="mx-auto max-w-2xl flex items-center gap-3 px-4 py-3">
          <Link href="/" className="rounded-lg p-2 hover:bg-stone-100 transition-colors">
            <ArrowLeft className="h-5 w-5 text-stone-600" />
          </Link>
          <h1 className="font-serif text-lg text-stone-900">Permohonan Doa</h1>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-2xl px-4 py-6">
        <motion.div
          ...
          className={`rounded-2xl bg-white p-6 shadow-sm ring-1 ${
            prayer.isAnswered ? "ring-emerald-200/80" : "ring-stone-100"
          }`}
        >
          {/* Author */}
          <div className="flex items-center gap-3 mb-5">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-stone-100 to-stone-200 text-base font-bold text-stone-500">
            ...
            {prayer.isAnswered && (
              <div className="ml-auto flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-600 ring-1 ring-emerald-200/50">
          ...
          {prayer.scriptureVerse && (
            <div className="mt-4 flex items-start gap-2 rounded-xl bg-amber-50/60 px-4 py-3 ring-1 ring-amber-100/60">
              <BookOpen className="h-4 w-4 mt-0.5 text-amber-600 shrink-0" />
              <p className="text-sm italic text-amber-800 leading-relaxed">{prayer.scriptureVerse}</p>
            </div>
          )}

          {/* Testimony */}
          {prayer.isAnswered && prayer.answeredTestimony && (
            <div className="mt-4 rounded-xl bg-emerald-50 px-4 py-3 ring-1 ring-emerald-100/60">
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            >
              <Heart className={`h-4 w-4 ${isPraying ? "fill-amber-500 text-amber-500" : ""}`} />
        ...
        <motion.div
          ...
          className="mt-5 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-stone-100"
        >
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-xl bg-stone-900 px-4 py-2.5 text-sm font-semibold text-white transition-all hover:bg-stone-800 active:scale-[0.98]"
            >
              <svg className="h-4 w-4 fill-current" viewBox="0 0 24 24">
            ...
            <button
              onClick={copyLink}
              className="flex items-center justify-center gap-2 rounded-xl bg-stone-100 px-4 py-2.5 text-sm font-semibold text-stone-700 transition-all hover:bg-stone-200 active:scale-[0.98] ring-1 ring-stone-200"
            >
