    ...
    <div className="min-h-screen bg-[#faf7f2] pb-24 md:pb-8">
      <header className="glass sticky top-0 z-40 border-b border-stone-200/60">
        <div className="mx-auto max-w-2xl flex items-center gap-3 px-4 py-3">
          ...
          <h1 className="font-serif text-lg text-stone-900">Partner Doa</h1>
        </div>
      </header>

      <main className="mx-auto max-w-2xl px-4 py-5 space-y-5">
        <div className="rounded-2xl bg-gradient-to-br from-rose-50 to-pink-50 p-5 ring-1 ring-rose-100/60 text-center">
          <Heart className="mx-auto h-8 w-8 text-rose-500 mb-2" />
          <h2 className="font-serif text-lg text-stone-900">Temukan Partner Doa</h2>
          <p className="text-sm text-stone-500 mt-1">Berdoa bersama partner yang saling mendukung dan menguatkan</p>
          <button onClick={handleMatch} disabled={matching}
            className="mt-4 inline-flex items-center gap-2 rounded-xl bg-stone-900 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50">
        ...
        {active.length > 0 && (
          <div>
            <h3 className="text-sm font-bold text-stone-700 mb-3">Partner Aktif</h3>
            <div className="space-y-2">
              {active.map(p => (
                <div key={p.id} className="flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-stone-100">
                  <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-stone-100">
                    {p.partnerImage ? <Image src={p.partnerImage} alt="" width={40} height={40} className="h-full w-full object-cover" /> :
                      <span className="text-sm font-bold text-stone-500">{(p.partnerName || "?")[0].toUpperCase()}</span>}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-stone-800">{p.partnerName || "Partner Doa"}</p>
        ...
        {pending.length > 0 && (
          <div>
            <h3 className="text-sm font-bold text-stone-700 mb-3">Menunggu Respon</h3>
            <div className="space-y-2">
              {pending.map(p => (
                <div key={p.id} className="flex items-center justify-between rounded-2xl bg-white p-4 shadow-sm ring-1 ring-stone-100">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-stone-100">
                      <span className="text-sm font-bold text-stone-500">{(p.partnerName || "?")[0].toUpperCase()}</span>
                    </div>
                    <p className="text-sm font-semibold text-stone-800">{p.partnerName || "Pengguna"}</p>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => handleRespond(p.id, "accept")} className="rounded-lg bg-emerald-50 p-2 text-emerald-600 hover:bg-emerald-100"><Check className="h-4 w-4" /></button>
                    <button onClick={() => handleRespond(p.id, "decline")} className="rounded-lg bg-red-50 p-2 text-red-500 hover:bg-red-100"><X className="h-4 w-4" /></button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {!loading && active.length === 0 && pending.length === 0 && (
          <div className="text-center py-10">
            <p className="text-sm text-stone-400">Belum ada partner doa. Klik tombol di atas untuk mencari!</p>
          </div>
        )}
      </main>
